...
def args_parser():
    ...
    parser.add_argument('--dataset', default='QM9', help='dataset name')
    ...
    parser.add_argument('--size_subgraph', type=int, default=5, help='size of the subgraph')
...
def main():
    ...
    # print('features list:', features_lst)
    ...
    n_classes = 1 #changed due to regression
    y = class_labels

    # print('Labels',y.shape)

    # print('Adj_lst:', adj_lst[0])
    ...
    random.shuffle(idx)
    r0 = int(N1 * train_ratio)
    r1 = int(N1 * 0.9)
    train_index = np.array(idx[:r0]).tolist()
    val_index = np.array(idx[r0:r1]).tolist()
    test_index = np.array(idx[r1:]).tolist()
    ...
    print('Total Graphs: ', n_train + n_test + n_val)
    ...
    print("n_test_batches", n_train_batches)
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(n_train_batches):
            ...
            train_acc.update(error(output.data, y_train[i].data), output.size(0))
        ...
        for i in range(n_val_batches):
            ...
            val_acc.update(error(output.data, y_val[i].data), output.size(0))
        ...
        if epoch % args.test_freq == 0: 
            ...
            for i in range(n_test_batches):
                ...
                test_acc.update(error(output.data, y_test[i].data), output.size(0))
